...
async function task1callback(req, response) {

  var task2uuid = 'e693e9b5-4719-4d1b-9204-c02ad3fe8eaf'
  var json = req;
  ...
  merge(['./pdf/first_step.pdf', './pdf/' + taskid + '.pdf'], './pdf/output.pdf', function (err) {
    ...
    axios.post('http://localhost:3016/api/initcli').then((res) => {
      ...
      axios.post('http://localhost:3016/api/sendTask', senddata).then((res) => {
        if (res.data.data.success === true) {
          ...
          axios.post('http://localhost:3016/api/addPDF', attachmentData).then((res) => {
            ...
            axios.post('http://localhost:3016/api/notify', notifydata).then((res) => {
...
function initClient(req, response) {
  ...
  axios.post(scheme + '://' + apiUrl + '/api/auth', data).then((res) => {
    ...
    if (status === 200) {
      ...
      return response.status(403).json({data:{success:false}});
    }
  });
    ...
    if (res.status === 200) {
      ...
      if (!src.done) {
        ...
        axios.request({
          ...
          //logger.info(err);
          return response.status(500).json({data:{success:false}});
        })
      } else {
        return response.status(500).json({data:{success:false}});
      }
...
function notifyTaskOwner(req, response/* task uuid */) {
  ...
  axios.request({
    ...
    if (status <= 204) {
      ...
      return response.status(404).json({data:{success:false}});
    }
  }).catch((err) => {
    logger.info(err)
  })
